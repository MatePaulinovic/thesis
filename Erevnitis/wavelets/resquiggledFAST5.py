...
import Erevnitis.proto.signal_pb2 as signal_pb2
import Erevnitis.proto.nanopolish_pb2 as  nanopolish_pb2
import Erevnitis.proto.raw_current_pb2 as raw_current_pb2
...
class ResquiggledFAST5():
    ...
    def get_nucleotide_positions(self, nucleotide):
        ...
        for event in events:
            if event.base == nucleotide:
                ...
                indices += list(range(event.start, event.start + event.length))
        return np.array(indices)


    #TODO
    def get_kmer_positions(self, kmer):
        """Returns all indices of positions where the specified kmer has been
        detected.
    ...
    def get_nucleotide_intervals(self, nucleotide):
        """Returns all start and end indices of intervals where the specified
        nucleotide has been detected.
    ...
    def get_nucleotide_mean_stdev(self, nucleotide):
        """Returns mean and standard deviation of the specified nucleotide in
        the signal.
    ...
    def get_channel_info(self):
        channel_id_attrs = self._key_dict_flat['channel_id'].attrs
        return ResquiggledFAST5._create_channelInfo(channel_id_attrs)
